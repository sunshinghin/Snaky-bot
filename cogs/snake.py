...
async def sendingembed(map,interaction,first,snake,score,moveemoji,WAIT_TIME,channel,author,hardmode):
  ...
  if first == True:
    ...
    embed_message = nextcord.Embed(title="Snake Game", description="React below to decide where the snake go!", color=nextcord.Color.red()) 
...
# create variables to save the highest reactions
  ...
  if selected == False:
    ...
    directiontoreturn = random.choice(direction_list)
...
class Snake:
  ...
  async def load_grid(self,channel,input,WAIT_TIME):
      ...
      if channel != None:
        ...
        await sendingembed("\n".join(maparray),channel,False,self,self.score,send_emoji,WAIT_TIME,channel,self.create_message_author,None)       
...
class snakestart(commands.Cog):
  ...
  async def playsnake(self, interaction: Interaction,snakehead,size: int = SlashOption(max_value=12,min_value=1,default=12,required=False),waittime: int = SlashOption(max_value=86400,min_value=1,default=2,required=False),hardmode: str = SlashOption(name="hardcore",choices=["y","n"],default="n",required=False),private: str = SlashOption(name="private", choices=["y","n"], default="n",required=False)):
    ...
    if set_command_channel_id == str(interaction.channel.id):
      ...
      if user_started_game == True:
        await interaction.response.send_message("You are already create a game!Cannot create new anymore!")
    ...
    self.client.user: nextcord.PermissionOverwrite(read_messages=True)
        ...
        await sendingembed(snake.return_grid(),interaction,True,snake,0,"🚫",waittime,channel,str(interaction.user.id),hardmode)
    else:
